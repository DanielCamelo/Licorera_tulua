...
const HorizontalCardProduct = ({ category, heading }) => {
    ...
    return (
        <div className='container mx-auto px-4 my-6 relative'>
            <h2 className='text-2xl font-semibold py-4 text-#4B0F0F'>{heading}</h2>
                ...
                {loading ? (
                    loadingList.map((_, index) => (
                        <div key={index} className='w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] h-36 bg-white rounded-sm shadow flex'>
                            ...
                            <div className='p-4 grid w-full gap-2'>
                                ...
                                <p className='capitalize text-red-500 p-1 bg-slate-200 animate-pulse rounded-full'></p>
                    ...
                    <>
                        ...
                        {data.map((product, index) => (
                            <Link key={index} to={`product/${product?._id}`} className='w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] h-36 bg-white shadow flex rounded-md border-2 border-red-900'>
                                <div className='bg-red-50 h-full p-4 min-w-[120px] md:min-w-[145px] rounded-md'>
                                ...
                                <div className='p-4 grid'>
                                    ...
                                    <div className='flex gap-3'>
                                        <p className='text-red-800 font-medium'>{displayCOPCurrency(product?.sellingPrice)}</p>
                                        {product?.price !== product?.sellingPrice && (
                                            <p className='text-slate-500 line-through'>{displayCOPCurrency(product?.price)}</p>
                                        )}
                                    </div>
                                    <button className='text-sm bg-red-800 hover:bg-red-900 text-white px-3 py-0.5 rounded-full' onClick={(e) => handleAddToCart(e, product?._id)}>
                                        Añadir al carrito
                                    </button>
                                </div>
                            </Link>
